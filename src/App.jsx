...
async function parsePptx(file) {
  ...
  while (true) {
    ...
    const slideText = textNodes
      ? textNodes.map((t) => t.replace(/<\/?a:t>/g, "")).join(" ").trim()
      : "";

    let title = `Slide ${i}`;
    const titleMatch = slideXml.match(
      /type="(?:title|ctrTitle)"[\s\S]*?<a:t>([\s\S]*?)<\/a:t>/
    );
    ...
    if (slideRelsFile) {
      ...
      const notesRelMatch = slideRelsXml.match(
      ...
      if (notesRelMatch) {
        ...
        if (notesFile) {
          ...
          // Look for shapes with the notes body placeholder: type="body"
          // Avoid: type="sldImg" (slide thumbnail), type="sldNum" (slide number),
          // type="dt" (date), type="ftr" (footer), type="hdr" (header)
          const shapes = notesXml.match(/<p:sp>[\s\S]*?<\/p:sp>/g);

          if (shapes) {
            for (const shape of shapes) {
              // Must have type="body" placeholder
              if (!/type\s*=\s*"body"/.test(shape)) continue;

              const bodyMatch = shape.match(
                /<p:txBody>[\s\S]*?<\/p:txBody>/
              );
              if (bodyMatch) {
                const extracted = extractTextParagraphs(bodyMatch[0]);
                // Skip if it's just a number (slide number) or empty
      ...
      // Slide thumbnail
      const imgMatch = slideRelsXml.match(
        /Target="\.\.\/media\/(image[^"]+\.(png|jpg|jpeg|gif|svg))"/i
      );
      if (imgMatch) {
...
function SlideCard({ slide, isPlaying, onPlay, onStop }) {
  ...
  return (
    <div style={{
      ...
      <div style={{
        height: 160,
        background: slide.imageData ? `url(${slide.imageData}) center/cover` : "linear-gradient(135deg, #1a1b24 0%, #262838 100%)",
        ...
        <div style={{
          position: "absolute", top: 10, left: 12, background: "rgba(0,0,0,0.6)",
          backdropFilter: "blur(8px)", padding: "3px 10px", borderRadius: 20,
          fontSize: 11, fontWeight: 700, color: C.text, fontFamily: "'JetBrains Mono', monospace",
        }}>
        ...
        <div style={{ marginTop: 12, display: "flex", gap: 8 }}>
          <button
            ...
            style={{
              ...
              background: isPlaying ? C.danger : hasNotes ? C.accent : C.border,
              color: hasNotes ? "#fff" : C.textMuted, fontSize: 12, fontWeight: 700,
              cursor: hasNotes ? "pointer" : "default", fontFamily: "'Space Grotesk', sans-serif",
              letterSpacing: "0.04em", transition: "all 0.2s ease",
              display: "flex", alignItems: "center", justifyContent: "center", gap: 6,
            }}
          >
            {isPlaying ? (<><span style={{ fontSize: 14 }}>■</span> STOP</>) : (<><span style={{ fontSize: 14 }}>▶</span> PLAY NOTES</>)}
          </button>
        </div>
      </div>
    </div>
...
export default function App() {
  ...
  const handleFile = async (e) => {
    ...
    try { setSlides(await parsePptx(file)); }
    catch (err) { console.error(err); alert("Failed to parse PPTX: " + err.message); }
    setLoading(false);
  };
  ...
  const playAll = useCallback(async () => {
    ...
    for (const slide of slides.filter((s) => s.notes)) {
  ...
  }, [slides, tts.selectedVoice, tts.rate]);
    ...
    <div style={{ minHeight: "100vh", background: C.bg, color: C.text, fontFamily: "'IBM Plex Sans', sans-serif" }}>
        ...
        <div>
          ...
          {fileName && (
            <div style={{ fontSize: 12, color: C.textMuted, marginTop: 4, fontFamily: "'JetBrains Mono', monospace" }}>
              {fileName} — {slides.length} slides — {slides.filter((s) => s.notes).length} with notes
            </div>
          )}
        </div>
        <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
          {slides.length > 0 && (
            <>
              ...
              <button onClick={() => setShowSettings(!showSettings)} style={{ padding: "8px 14px", borderRadius: 8, border: `1px solid ${C.border}`, background: showSettings ? C.surfaceHover : "transparent", color: C.textMuted, fontSize: 12, cursor: "pointer", fontFamily: "'Space Grotesk', sans-serif", fontWeight: 600 }}>
                ⚙ Voice
              </button>
      ...
      {showSettings && (
        <div style={{ padding: "16px 32px", borderBottom: `1px solid ${C.border}`, background: C.surface, display: "flex", gap: 24, alignItems: "center", flexWrap: "wrap", animation: "fadeUp 0.25s ease" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <label style={{ fontSize: 12, color: C.textMuted, fontWeight: 600, fontFamily: "'Space Grotesk', sans-serif" }}>Voice:</label>
            <select value={tts.selectedVoice?.name || ""} onChange={(e) => { const v = tts.voices.find((x) => x.name === e.target.value); if (v) tts.setSelectedVoice(v); }}
              style={{ background: C.bg, color: C.text, border: `1px solid ${C.border}`, borderRadius: 6, padding: "6px 10px", fontSize: 12, fontFamily: "'IBM Plex Sans', sans-serif", maxWidth: 260 }}>
              {tts.voices.map((v) => (<option key={v.name} value={v.name}>{v.name} ({v.lang})</option>))}
            </select>
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <label style={{ fontSize: 12, color: C.textMuted, fontWeight: 600, fontFamily: "'Space Grotesk', sans-serif" }}>Speed: {tts.rate.toFixed(1)}x</label>
      ...
      {slides.length === 0 && !loading && (
        <div style={{ display: "flex", alignItems: "center", justifyContent: "center", minHeight: "70vh", padding: 32 }}>
          <label style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", width: 420, height: 280, border: `2px dashed ${C.border}`, borderRadius: 20, cursor: "pointer", transition: "all 0.3s ease", background: C.surface }}
      ...
      {loading && (
        <div style={{ display: "flex", alignItems: "center", justifyContent: "center", minHeight: "60vh", fontSize: 15, color: C.textMuted, animation: "pulse 1.2s ease infinite", fontFamily: "'Space Grotesk', sans-serif" }}>
          Parsing slides...
        </div>
      )}
        ...
        <div style={{ padding: "28px 32px", display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: 20 }}>
          {slides.map((slide, idx) => (
            <div key={slide.index} style={{ animation: `fadeUp 0.4s ease ${idx * 0.05}s both` }}>
              <SlideCard slide={slide} isPlaying={tts.speaking === slide.index} onPlay={() => tts.speak(slide.notes, slide.index)} onStop={() => tts.stop()} />
            </div>
          ))}
        </div>
      )}
